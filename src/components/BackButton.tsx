...
export default function BackButton({
  ...
  const handleNavigateToEntry = React.useCallback((entry: typeof previousEntries[number]) => {
    try {
      setMenuOpen(false);
      navigateTo(entry);
    } catch (error) {
  ...
  const handlePrimaryClick = React.useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
    onClick?.(event);
  ...
  const handleClearHistory = React.useCallback((event: Event) => {
  ...
  return <div className={cn("inline-flex items-stretch", wrapperClassName)}>
      
      {showHistoryMenu ? <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
          <DropdownMenuTrigger asChild>
            
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" sideOffset={8} className="flex max-h-[min(70vh,26rem)] w-[min(85vw,22rem)] flex-col gap-1.5 overflow-y-auto rounded-lg border border-border/60 bg-popover p-2 text-popover-foreground shadow-lg sm:w-[22rem] sm:p-3">
            <DropdownMenuLabel className="flex flex-col gap-1 rounded-md bg-muted/40 px-3 py-2 text-left !font-normal">
              ...
              {currentFullPath ? <span className="text-xs text-muted-foreground break-all">{currentFullPath}</span> : null}
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            {hasHistory ? previousEntries.map(entry => {
          const fullPath = `${entry.pathname ?? ""}${entry.search ?? ""}${entry.hash ?? ""}`;
          return <DropdownMenuItem key={entry.id} onSelect={event => {
            event.preventDefault();
          ...
          setMenuOpen(false);
            ...
            <DropdownMenuItem onSelect={handleClearHistory} className="flex items-center gap-2 px-2 py-2 text-sm text-destructive hover:translate-x-0 focus:text-destructive focus:translate-x-0 sm:px-3">

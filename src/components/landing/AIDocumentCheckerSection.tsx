...
const fallbackApprovals = ["Passport", "WAEC/NECO", "Transcripts", "Recommendation letters", "Bank statements"];
const fallbackDetections = ["Missing pages", "Unclear images", "Wrong document type", "Fraud signs"];
const fallbackRiskSignals = ["Same passport used for multiple accounts", "Students buying fabricated bank statements", "Uploading fake WAEC results", "Agents sending unrealistic profiles"];
const fallbackStats: StatItem[] = [{
  value: "60s",
  label: "Average review time"
}, {
  value: "5 docs",
  label: "Checked simultaneously"
}, {
  value: "24/7",
...
const parseStringArray = (value: unknown): string[] | null => {
  ...
  const parsed = value.map(item => typeof item === "string" ? item : null).filter((item): item is string => Boolean(item));
...
function AIDocumentCheckerSection() {
  ...
  const approvals = useMemo(() => parseStringArray(t("pages.index.aiDocumentChecker.approvals.items", {
    returnObjects: true
  }) as unknown) ?? fallbackApprovals, [t]);
  const detections = useMemo(() => parseStringArray(t("pages.index.aiDocumentChecker.detections.items", {
    returnObjects: true
  }) as unknown) ?? fallbackDetections, [t]);
  const stats = useMemo(() => parseStats(t("pages.index.aiDocumentChecker.stats", {
    returnObjects: true
  }) as unknown) ?? fallbackStats, [t]);
  const riskSignals = useMemo(() => parseStringArray(t("pages.index.aiDocumentChecker.riskMonitoring.items", {
    returnObjects: true
  }) as unknown) ?? fallbackRiskSignals, [t]);
  return <section className="container mx-auto px-4 py-20">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        <Card className="border-primary/20 bg-gradient-to-b from-primary/5 via-background to-background">
          ...
          <CardContent>
            <ul className="space-y-3">
              {approvals.map(item => <li key={item} className="flex items-center gap-3 rounded-2xl border border-primary/10 bg-background/80 p-3">
        ...
        <Card className="border-red-200/40 bg-gradient-to-b from-background via-background to-red-50/50 dark:to-background">
          ...
          <CardContent>
            <ul className="space-y-3">
              {detections.map(item => <li key={item} className="flex items-center gap-3 rounded-2xl border border-red-200/40 bg-background/80 p-3">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {stats.map(stat => <Card key={stat.value} className="border-muted-foreground/10 bg-background/80">
            <CardHeader>

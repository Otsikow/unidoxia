...
/* ---------- Skeleton Loader ---------- */
const SectionLoader = () => <div className="container mx-auto px-4 py-20">
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {Array.from({
        length: 3
      }).map((_, i) => <div key={i} className="h-64 bg-muted animate-pulse rounded-xl" />)}
      </div>
    </div>
  </div>;
const Index = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background via-background to-muted/20">
      ...
      <section className="container mx-auto px-4 py-20">
        ...
        {faqs.map((section, i) => <div key={i} className="max-w-4xl mx-auto mb-12">
            ...
            <Accordion type="single" collapsible>
              {section.items.map((faq, j) => <AccordionItem key={j} value={`${i}-${j}`}>

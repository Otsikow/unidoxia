  ...
  const financeQuery = useQuery({
    ...
    queryFn: async (): Promise<{ commissions: CommissionWithRelations[]; payments: CommissionPayment[] }> => {
      const [commissionsResult, paymentsResult] = await Promise.all([
        supabase.from("commissions").select(`*, agents:agents ( profiles:profiles!agents_profile_id_fkey ( full_name ) ), applications:applications ( students:students ( profiles:profiles!students_profile_id_fkey ( full_name ) ) )`).order("created_at", { ascending: false }),

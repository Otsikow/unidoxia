...
const datasetPath = new URL("../../data/catalogues/lambton-college-reviewed.json", import.meta.url);
const outputPath = new URL("../../supabase/migrations/20260814124500_lambton_college_programmes.sql", import.meta.url);
...
const feeBasis = (value) => value === "programme" ? "total" : (value ?? "total");
const uuid = (key) => {
  const hash = createHash("sha256").update(`lambton-college:${key}`).digest("hex");
...
for (const programme of dataset.programmes) {
  ...
  ${quote(programme.tuition?.currency ?? "CAD")}, ${programme.tuition?.amount ?? "NULL"}, ${quote(`{${programme.intakes.map((intake) => intake.month).join(",")}}`)}::integer[],
  ${json(programme.requirements)}, NULL, ${quote(programme.overview)}, ${bool(programme.catalogueStatus === "active")}, ${json(programme.requirements)},
  ${quote(programme.qualification)}, ${quote(programme.faculty)}, ${quote(programme.courseCode)}, ${quote(programme.studyMode)}, ${quote(programme.attendance)},
  ...
  ${quote(programme.tuition?.feeYear)}, ${quote(programme.tuition ? feeBasis(programme.tuition.feeBasis) : null)}, ${bool(programme.tuition?.amount != null)}, ${quote(programme.catalogueStatus)},
  'official_source_verified', ${quote(programme.classification || (programme.tuition?.amount == null ? "verified_fee_pending" : "verified_current"))},
  ${quote(dataset.checkedAt)}, now(), ${quote(sourceFingerprint(programme))}, ${quote(programme.overview)}, ${json(programme.modules)},
  ${quote(programme.careerOutcomes)}, ${quote(programme.accreditation)}, ${json(programme.applicationDetails)}, ${json(programme.englishRequirements)}
)
...
FROM public.programs WHERE university_id = (SELECT id FROM public.universities WHERE slug = 'lambton-college') AND official_url = ${quote(programme.officialUrl)}
...
SELECT id, 'international', ${programme.tuition.amount ?? "NULL"}, ${quote(programme.tuition.currency ?? "CAD")}, ${quote(programme.tuition.feeYear)}, ${quote(feeBasis(programme.tuition.feeBasis))}, ${json(programme.tuition.terms ?? [])}, ${quote(programme.tuition.amount == null ? "unresolved" : "verified")}, ${quote(programme.tuition.sourceUrl)}, ${quote(dataset.checkedAt)}
FROM public.programs WHERE university_id = (SELECT id FROM public.universities WHERE slug = 'lambton-college') AND official_url = ${quote(programme.officialUrl)}

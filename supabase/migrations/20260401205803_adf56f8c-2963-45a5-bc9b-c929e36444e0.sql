...
DECLARE
  ...
  new_role app_role;
BEGIN
  -- CRITICAL: Use the correct default tenant by slug, not random LIMIT 1
  -- This prevents agents/students from being assigned to isolated university tenants
  ...
  -- Fallback: if no known default tenant, pick the earliest created one
  ...
  IF new_role = 'student'::app_role THEN
    INSERT INTO public.students (tenant_id, profile_id, referral_source)
    VALUES (
      default_tenant_id, 
      NEW.id,

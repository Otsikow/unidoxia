...
-- 1. The buggy migration failed entirely (no function, no policies)
-- 2. The buggy migration partially applied (policies dropped but function not created)
-- 3. Old policies from earlier migrations still exist
-- 4. This migration has already been applied

-- Drop and recreate the function with correct role checks
-- Using CREATE OR REPLACE ensures this works whether function exists or not
...
BEGIN
  ...
  -- Check 1: UniDoxia internal roles (admin/staff can access all submitted applications)
  IF public.is_admin_or_staff(p_user_id) THEN
    RETURN true;
  END IF;

  -- Check 2: Student who owns the application (uses denormalized student_profile_id for efficiency)
  -- Also fall back to checking via students table if student_profile_id is NULL
  SELECT true INTO v_has_access
  FROM public.applications a
  WHERE a.id = p_application_id
    AND a.submitted_at IS NOT NULL
    AND (
      a.student_profile_id = p_user_id
      OR EXISTS (
        SELECT 1 FROM public.students s
        WHERE s.id = a.student_id AND s.profile_id = p_user_id
      )
    );
  ...
  -- Check 3: Assigned/referring agent
  SELECT true INTO v_has_access
  FROM public.applications a
  ...
  -- Check 4: University partner (tenant-scoped) for submitted applications
  -- Note: 'university' is not a valid app_role enum value - university users have role 'partner'
  IF public.has_role(p_user_id, 'partner'::public.app_role) 
     OR public.has_role(p_user_id, 'school_rep'::public.app_role) THEN
    SELECT true INTO v_has_access
    FROM public.applications a
...
-- Drop ALL possible policy names to ensure clean state
-- This handles cases where:
-- 1. The original migration created old-style policies
-- 2. The buggy migration dropped them but didn't create new ones
-- 3. Any intermediate state

-- ============================================================================
-- Ensure `public.update_application_review` is visible + executable via PostgREST
-- ============================================================================
-- Production symptom:
--   PostgREST/Supabase JS throws:
--     "Could not find the function public.update_application_review(...) in the schema cache"
--
-- Root causes addressed here:
-- - Function exists but PostgREST schema cache hasn't reloaded yet
-- - Function exists but is not executable for the caller's DB role (anon/authenticated),
--   which makes PostgREST treat it as "not found"
--
-- This migration:
-- - Recreates the RPC with the exact parameter names used by the frontend
-- - Ensures enum value 'rejected' exists (UI supports it)
-- - Grants EXECUTE to both authenticated and anon
--   (anon calls will still fail fast with "Not authenticated" inside the function)
-- - Notifies PostgREST to reload schema cache immediately
-- ============================================================================

-- 1) Ensure enum includes UI-supported values
DO $$
BEGIN
...
-- 2) Create/replace RPC (keep signature stable for frontend)
...
-- 3) Ensure PostgREST can see + execute the function for both session states
...
-- 4) Force PostgREST to reload schema cache so RPC is immediately callable
